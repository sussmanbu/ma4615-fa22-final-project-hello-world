...
After observing the relationships between variables through EDA, 
...
We then create the scatterplot matrix of all the variables, figuring out which variables have the biggest correlation with Medicare_Coverage. According to the graph, BENE_AVG_RISK_SCRE seems like the one with the highest correlation with Medicare Coverage, with 0.416. Los and ACUTE_HOSP_READMSN_PCT are another variables that also indicate strong relationships with Medicare Coverage.
...
We first fit a linear regression model on our output Medicare_Coverge with all other x variables. We get a F-statistics of 2.05 p-value of 0.06286, which is roughly close to 0.05. The adjusted R-squared of 0.1443, which says that 14.43% of the variance in the training set is explained by the model. According to the t-test, most of the p-values are greater than 0.05, such as those of GPCI, indicating that they have little impact of Medicare Coverage. 
```{r}
